...
function getLeaderboard() {
    ...
    return data ? JSON.parse(data) : [
        {name: "DR_ALPHA", score: 2500}, {name: "SUBJ_89", score: 1800},
        {name: "HAZMAT_01", score: 1200}, {name: "LAB_TECH", score: 600}, {name: "INTERN", score: 200}
    ];
}
...
function openAdModal() {
    ...
    // Solo carga el iframe si AD_LINK es una URL real (evita que "" o "#" recarguen esta misma página adentro del modal).
    const isValidAdLink = /^https?:\/\//i.test(AD_LINK);
    if (isValidAdLink) {
        frame.src = AD_LINK;
    ...
    let secondsLeft = 5;
    btn.disabled = true;
    btn.innerText = `CERRAR EN ${secondsLeft}s`;
    ...
    adCountdownInterval = setInterval(() => {
        ...
        if (secondsLeft <= 0) {
            ...
            btn.disabled = false;
            btn.innerText = 'CERRAR Y CONTINUAR';
        } else {
            btn.innerText = `CERRAR EN ${secondsLeft}s`;
        }
    }, 1000);
...
function closeAdAndContinue() {
    ...
    document.getElementById('adFrame').src = 'about:blank'; // detiene cualquier audio/video del anuncio
...
const AD_LINK = ""; // TODO: reemplazar con el link real del anuncio (debe ser una URL http(s) válida, no "#")

let player = {
    worldX: 0, worldY: 0, screenX: canvas.width / 2, screenY: canvas.height / 2,
    radius: 12, baseSpeed: 3.0, speed: 3.0, hp: 100, maxHp: 100,
    level: 1, dna: 0, dnaNeeded: 5,
    mutations: { acido: 0, puas: 0, alas: 0, regeneracion: 0, pulsoMejora: 1 },
    pulseCooldown: 0, pulseActiveVisual: 0, kameTimer: 0, invulnUntil: 0
};
...
const MUTACIONES_DATA = [
    { id: 'acido', nombre: 'Brazo Químico', desc: 'Tu brazo desgarra la bata y escupe ácido a los infectados cercanos.' },
    { id: 'puas', nombre: 'Exoesqueleto Reactivo', desc: 'Púas de quitina atraviesan tu traje para devolver daño letal.' },
    { id: 'alas', nombre: 'Alas de Energía', desc: 'Energía pura brota de tu espalda, aumentando drásticamente tu velocidad.' },
    { id: 'regeneracion', nombre: 'Sangre Mitótica', desc: 'Tu cuerpo se cura a sí mismo cerrando heridas automáticamente.' },
    { id: 'pulsoMejora', nombre: 'Sobrecarga de Traje', desc: 'Amplía el radio y la fuerza del empuje electromagnético [Espacio].' }
];
...
function resetGameState() {
    ...
    player.worldX = 0; player.worldY = 0; player.hp = 100; player.level = 1; player.dna = 0; player.dnaNeeded = 5;
    player.speed = player.baseSpeed; player.kameTimer = 0; player.invulnUntil = 0;
...
function showMutationMenu() {
    ...
    // 3. Crear y adjuntar los elementos al DOM uno por uno (más seguro que innerHTML masivo en bucle)
    MUTACIONES_DATA.forEach(m => {
        ...
        btn.innerHTML = `<strong>${txtBoton}</strong><span class="mut-desc">${m.desc}</span>`;

        // Manejador de click seguro
        btn.onclick = (e) => {
            e.preventDefault();
            aplicarMutacion(m.id, m.nombre);
        };

        container.appendChild(btn);
    });

    // 4. Mostrar el modal una vez el árbol de botones esté completamente listo en el buffer
    modal.style.display = 'block';
}

function aplicarMutacion(id, nombre) {
